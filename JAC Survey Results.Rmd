...
AquaAmenities.dat <- ResidentSurvey.dat %>%
  ...
  select(-last_col()) %>%  # Remove the usage rate column
...
ggplot(count_data, aes(x = factor(AquaticAmenity, levels = ordered_amenities), 
                       y = Percentage, 
                       fill = Ranking)) +
  geom_bar(stat = "identity", position = "stack") +
  scale_fill_viridis(discrete = TRUE) +  # Use the Viridis color palette for discrete categories
  labs(title = "Aquatic Amenities: Ranking Categories by Amenity",
       x = "Aquatic Amenity",
       y = "Percentage (%)",
       fill = "Ranking") +
  theme_minimal() +
